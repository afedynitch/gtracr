...
RIGIDITY_FORBIDDEN = 5.   # GV — forbidden (particle returns to atmosphere)
RIGIDITY_ALLOWED   = 30.  # GV — well above cutoff (particle escapes)
BFIELD_TYPE  = "igrf"
...
def section_accuracy(show_plots):
    ...
    data_ref, traj_ref, _ = run_trajectory("rk45", rigidity=RIGIDITY_FORBIDDEN,
                                            dt=DT_REF, atol=1e-8, rtol=1e-10)
    r_final_ref  = traj_ref.final_sixvector[0]
    th_final_ref = traj_ref.final_sixvector[1]
    ph_final_ref = traj_ref.final_sixvector[2]
    ...
    for tol in tol_vals:
        d, tr, _ = run_trajectory("rk45", rigidity=RIGIDITY_FORBIDDEN,
                                   dt=DT_REF, atol=tol, rtol=tol)
        dr, dth = _err(tr)
        n = len(d["t"])
...
def section_energy_conservation(show_plots):
    ...
    solvers = [("rk4", "RK4 (frozen-field)", "C0"),
    ...
    for key, label, _ in solvers:
        ...
        t_arr = d["t"]
        pr    = d["pr"];  pth = d["ptheta"];  pph = d["pphi"]
...
def section_performance(n_perf, show_plots):
    ...
    traj0 = Trajectory(
        plabel=PLABEL, location_name=loc_name,
        zenith_angle=45., azimuth_angle=90.,
        rigidity=RIGIDITY_FORBIDDEN, bfield_type=BFIELD_TYPE, solver="rk4",
    )
    ...
    for sname, schar in [("rk4", "r"), ("boris", "b"), ("rk45", "a")]:
        tracer = CppTrajectoryTracer(
            charge_si, mass_si,
            float(traj0.start_alt), float(traj0.esc_alt),
            DT_REF, int(max_step), traj0.bfield_type,
            traj0.igrf_params,
            schar, 1e-3, 1e-6,
        )
        total_steps = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-perf", dest="n_perf", type=int, default=200,
                        help="Number of trajectories for the performance benchmark (default 200).")
    args = parser.parse_args()
    show_plots = not args.no_plots

    print("gtracr solver comparison")
    print(f"  particle   : {PLABEL}, R_forbidden={RIGIDITY_FORBIDDEN} GV / R_allowed={RIGIDITY_ALLOWED} GV")
