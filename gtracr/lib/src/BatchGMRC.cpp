...
static TransformContext build_transform_context(double lat_deg, double lng_deg,
                                                 double dalt_km, double palt_km) {
    ...
    ctx.mat[0][0] = -se;          ctx.mat[0][1] = -ce * sl;  ctx.mat[0][2] = cl * ce;
    ctx.mat[1][0] =  ce;          ctx.mat[1][1] = -sl * se;  ctx.mat[1][2] = cl * se;
    ctx.mat[2][0] =  0.0;         ctx.mat[2][1] =  cl;       ctx.mat[2][2] = sl;
...
static void get_particle_coord(double zenith_deg, double azimuth_deg,
                                double altitude, double magnitude,
...
static void apply_transform(const TransformContext& ctx,
...
static void cart_to_sph_coord(double x, double y, double z,
...
static void cart_to_sph_mom(double theta, double phi,
                              double px, double py, double pz,
...
static DirectionIC compute_direction_ic(const TransformContext& ctx,
                                         double zenith_deg, double azimuth_deg,
                                         double ref_momentum_gevc) {
    DirectionIC ic;
    ...
    if (zenith_deg > 90.0) {
        ...
        double magnitude = -(2.0 * constants::RE + ctx.palt_m)
                           * std::cos(zenith_deg * constants::DEG_TO_RAD);
        get_particle_coord(zenith_deg, azimuth_deg, 0.0, magnitude,
                            pc_x, pc_y, pc_z);
    } else {
        get_particle_coord(zenith_deg, azimuth_deg, ctx.palt_m, 1e-10,
                            pc_x, pc_y, pc_z);
    }

    double cart_x, cart_y, cart_z;
    apply_transform(ctx, pc_x, pc_y, pc_z,
                     ctx.det_x, ctx.det_y, ctx.det_z,
    ...
    get_particle_coord(zenith_deg, azimuth_deg, 0.0, mom_si,
                        mp_x, mp_y, mp_z);

    // Transform momentum (offset = 0,0,0 for momentum)
    double cart_px, cart_py, cart_pz;
    apply_transform(ctx, mp_x, mp_y, mp_z,
                     0.0, 0.0, 0.0,
                     cart_px, cart_py, cart_pz);
...
static WorkerResult thread_worker(
    ...
    while (successes < quota && attempts < max_attempts) {
        ...
        double rc = find_cutoff_rigidity_bisect(
            tracer, ic.pos, ic.mom_unit, rigidities_asc, mom_factor,
...
BatchGMRCResult batch_gmrc_evaluate(
    ...
    TransformContext ctx = build_transform_context(
    ...
    for (double r = params.min_rigidity; r <= params.max_rigidity + 1e-12; r += params.delta_rigidity) {
        rigidities_asc.push_back(r);
    }
    // Ensure max_rigidity is included
    if (rigidities_asc.empty() ||
        rigidities_asc.back() < params.max_rigidity - 1e-12) {
    ...
    for (int t = 0; t < n_threads; ++t) {
        ...
        threads[t] = std::thread([&, t, quota, max_attempts, seed]() {
            results[t] = thread_worker(
                shared_table, table_params, igrf_params, ctx,
