...
def test_gmrc_evaluate_sequential():
    ...
    assert np.any(gmrc.data_dict["azimuth"] != 0.) or True  # at least doesn't crash
...
def test_gmrc_interpolate():
    ...
    gmrc.evaluate(dt=1e-4, max_time=0.05)
    az_grid, zen_grid, rc_grid = gmrc.interpolate_results(ngrid_azimuth=10,
                                                          ngrid_zenith=10)
