...
def test_particle_set_from_rigidity():
    ...
    p.set_from_rigidity(10.)
    assert np.isclose(p.rigidity, 10.)
    assert np.isclose(p.momentum, 10.)  # charge = 1


def test_particle_set_from_energy():
    '''Proton with E=10 GeV: momentum = sqrt(E^2 - m^2)'''
    p = Particle("proton", 2212, 0.937272, 1, "p+")
    p.set_from_energy(10.)
    expected_momentum = np.sqrt(10.**2 - 0.937272**2)
...
def test_particle_set_from_momentum():
    ...
    p = Particle("proton", 2212, 0.937272, 1, "p+")
    p.set_from_momentum(5.)
    assert np.isclose(p.momentum, 5.)
    assert np.isclose(p.rigidity, 5.)
...
def test_particle_get_energy_rigidity():
    ...
    expected = np.sqrt((10. * 1)**2 + 0.937272**2) + 0.937272
...
def test_location_str():
    ...
    loc = Location("Kamioka", 36.4348, 137.2766, 0.)
    s = str(loc)
    assert "Kamioka" in s
    assert "36.4348" in s
