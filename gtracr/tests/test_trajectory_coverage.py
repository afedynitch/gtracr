...
from gtracr.lib.constants import EARTH_RADIUS, ELEMENTARY_CHARGE, KG_PER_GEVC2
...
def test_energy_input():
    ...
    assert traj.energy == 10.
    assert traj.rigidity > 0.
...
def test_convert_to_cartesian():
    ...
    data = {"r": np.array([EARTH_RADIUS]), "theta": np.array([np.pi / 2.]),
            "phi": np.array([0.])}
...
def test_geodesic_to_cartesian():
    ...
    assert np.isclose(cart[1], 0., atol=1.)
    assert np.isclose(cart[2], 0., atol=1.)
...
def test_transform_matrix_identity():
    ...
    assert np.isclose(mat[2, 0], 0.)
    assert np.isclose(mat[2, 1], 1.)
    assert np.isclose(mat[2, 2], 0.)
...
def test_cartesian_to_spherical():
    ...
    R = EARTH_RADIUS * 2.
    cart_coord = np.array([R, 0., 0.])
    cart_mmtm = np.array([0., 0., 0.])
    sv = traj.cartesian_to_spherical(cart_coord, cart_mmtm)
    assert np.isclose(sv[0], R)  # r
    assert np.isclose(sv[1], np.pi / 2., atol=1e-10)  # theta = pi/2 for z=0
    assert np.isclose(sv[2], 0., atol=1e-10)  # phi = arctan2(0,R) = 0
...
def test_ptrajectorytracer_dipole():
    ...
    escape_radius = 10. * EARTH_RADIUS  # escape at 10 RE
    ...
    vec0 = np.array([start_r, np.pi / 2., 0., momentum_si, 0., 0.])
    tracer.evaluate(0., vec0)
    assert tracer.final_time > 0.
...
def test_ptrajectorytracer_get_trajectory():
    ...
    vec0 = np.array([start_r, np.pi / 2., 0., momentum_si, 0., 0.])
    data = tracer.evaluate_and_get_trajectory(0., vec0)
    for key in ["t", "r", "theta", "phi", "pr", "ptheta", "pphi"]:
        assert key in data
...
def test_ptrajectorytracer_igrf():
    ...
    start_altitude_m = 100e3
    escape_radius = 10. * EARTH_RADIUS
    momentum_si = 0.937272 * 5.36e-19 * 10.  # higher momentum
...
def test_igrf13_leap_year():
    ...
    bf = pyigrf.values(EARTH_RADIUS, np.pi / 2., np.pi)
    assert len(bf) == 3
...
def test_igrf13_large_phi():
    ...
    bf = pyigrf.values(EARTH_RADIUS, np.pi / 2., phi_large)
    assert len(bf) == 3
...
def test_ptrajectorytracer_escaped():
    ...
    momentum_si = 50. * KG_M_S_PER_GEVC  # large outward momentum
...
def test_ptrajectorytracer_get_traj_escaped():
    ...
    start_altitude_m = 100e3
    small_escape = 300e3
    momentum_si = 50. * KG_M_S_PER_GEVC
